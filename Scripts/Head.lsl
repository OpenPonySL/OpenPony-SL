...
processtrigger(string trigger, string source)
{
    ...
    string triggerdata = llLinksetDataRead("trig-" + trigger);

    //integer found = llListFindList(triggerarray,[trigger]);
    //if(~found) triggerdata = llList2String(triggerarray,found+1);
...
default
{
    ...
    state_entry()
    {
        //llOwnerSay(llDumpList2String(llLinksetDataListKeys( 0, 50),"\n"));
    ...
    listen(integer chan, string name, key id, string mess)
    {
        if (id != Owner)
        {
            ...
            string triggerdata = llLinksetDataRead("trig-" + mess);
            //string triggerdata;
            /*
            integer found = llListFindList(triggerarray,[mess]);
            if(~found) triggerdata = llList2String(triggerarray,found+1);
            */
            if (triggerdata == "") return;
    ...
    { //(currentnotecardindex+1) != llGetInventoryNumber(INVENTORY_NOTECARD)
        ...
        else
        {
            while (Data != EOF)
            {
                ...
                while (length--)
                {
                    ...
                    if (trigger != "")
                    {
                        integer status = llLinksetDataWrite("trig-" + trigger, datatowrite);
                        //triggerarray += [trigger,datatowrite];
                        /*
